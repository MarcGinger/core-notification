  ...
  async catchup<T>(
    ...
    try {
      ...
      for await (const resolved of eventsPage) {
        ...
        const eventType = resolved.event.type;
        const occurredAt = new Date(resolved.event.created);

        const isLive = false;
        const evt = resolved.event.data as T;
        const eventRevision = resolved.event.revision; // Use revision within the stream for deduplication

        const metadata: EventStoreMetaProps = {
          ...(resolved.event.metadata as unknown as EventStoreMetaProps),
          stream: resolved.event.streamId, // Add stream name for deduplication
          revision: eventRevision, // Use revision for deduplication (kept same name for compatibility)
        };

        onEvent(evt, metadata);
      }
    ...
    const startSubscription = () => {
      ...
      esdbSubscription.on('data', (resolved: ResolvedEvent) => {
        ...
        const metadata: EventStoreMetaProps = {
          ...(resolved.event.metadata as unknown as EventStoreMetaProps),
          stream: resolved.event.streamId, // Add stream name for deduplication
          revision, // Use revision for deduplication (kept same name for compatibility)
        };
