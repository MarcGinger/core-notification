...
import { MessageQueueWorkerServiceConstants } from './service-constants';

/**
 * Domain value object for Message projection keys
 ...
 */
export class MessageQueueProjectionKeys {
  // EventStore DB patterns
  static readonly ESDB_BOUNDED_CONTEXT =
    MessageQueueWorkerServiceConstants.BOUNDED_CONTEXT;
  static readonly ESDB_AGGREGATE_NAME = 'message';
  static readonly ESDB_VERSION = 'v1';

  /**
   * Get EventStore stream prefix for individual streams
   * Format: core-slack.message.v1
   */
   ...
   * Format: $ce-core-slack.message.v1
   */
   ...
   * Format: core-slack.message.v1-{tenant}-{code}
   */
   ...
   * Validates format: core-slack.message.v1-{tenant}-{code}
   */
  ...
  /**
   * Validate if stream name matches message pattern
   */
  static isMessageStream(streamName: string): boolean {
    return this.extractFromStreamName(streamName) !== null;
  }

  /**
   * Get stream pattern for tenant-specific catchup
   * Format: core-slack.message.v1-{tenant}-*
   */
