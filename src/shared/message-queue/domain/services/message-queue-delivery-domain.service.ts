...
/**
 * Domain service for MessageQueue delivery business rules and error classification
 * Contains the core business logic for determining how to handle MessageQueue API errors
 * and retry strategies based on domain knowledge.
 */
@Injectable()
export class MessageQueueDeliveryDomainService {
  /**
   * Classifies a MessageQueue API error as either permanent (don't retry) or retryable
   * This encapsulates the business rules around which MessageQueue errors should be retried
   */
  classifyMessageQueueError(messageError: string): 'permanent' | 'retryable' {
    ...
    const permanentErrors = [
      ...
      'account_inactive', // MessageQueue account is deactivated
      'token_revoked', // OAuth token has been revoked
      'invalid_auth', // Authentication credentials are invalid
      'msg_too_long', // MessageQueue exceeds MessageQueue's length limits
    ];
    ...
    const retryableErrors = [
      ...
      'internal_error', // MessageQueue internal error - temporary
      'service_unavailable', // MessageQueue service temporarily down
    ];
   ...
   */
  generateUserFriendlyErrorMessageQueue(
    messageError: string,
    channel: string,
  ): string {
    const errorMessageQueues: Record<string, string> = {
      ...
      account_inactive: `MessageQueue account is inactive and cannot receive messages`,
      token_revoked: `MessageQueue authentication token has been revoked. Please reconfigure the integration`,
      invalid_auth: `MessageQueue authentication failed. Please check the bot token configuration`,
      msg_too_long: `MessageQueue is too long for MessageQueue. Please shorten the message content`,
    };

    return (
      errorMessageQueues[messageError] ||
      `MessageQueue delivery failed: ${messageError}`
    );
  }
