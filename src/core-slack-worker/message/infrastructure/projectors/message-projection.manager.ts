...
import { SendSlackMessageEventHandler } from '../event-handlers';
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.log(
        logContext,
        'Starting Slack message event subscription manager initialization',
      );
      ...
      this.logger.log(
        logContext,
        'Slack message event subscription manager initialized successfully',
      );
    } catch (error) {
      const errorContext =
        CoreSlackWorkerLoggingHelper.createEnhancedLogContext(
          'MessageProjectionManager',
          'onModuleInit',
          undefined,
          undefined,
          {
  ...
  async startProjection(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        { ...setupContext, subscriptionEstablished: true },
        'EventStore subscription established successfully',
      );
      ...
      this.logger.log(
        { ...setupContext, handlerInitialized: true },
        'Event handler marked as initialized and ready',
      );
      ...
      this.logger.log(
        successContext,
        'EventStore subscription setup completed successfully - ready to receive events',
      );
    } catch (error) {
  ...
  private async handleMessageEvent(
    ...
    // Add debug logging to track when events are received
    const debugContext = CoreSlackWorkerLoggingHelper.createEnhancedLogContext(
      'SlackMessageEventSubscriptionManager',
      'handleMessageEvent',
      event?.correlationId,
      undefined,
    ...
    try {
      ...
      this.logger.debug(
        debugContext,
        'Event successfully processed by handler',
      );
    } catch (error) {
      const errorContext =
        CoreSlackWorkerLoggingHelper.createEnhancedLogContext(
