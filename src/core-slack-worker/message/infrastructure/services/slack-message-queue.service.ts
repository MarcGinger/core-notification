...
export class SlackMessageQueueService {
  ...
  async addSlackMessageJob(
    ...
    return this.slackMessageQueue.add('send-slack-message', messageData, {
      priority: options?.priority || SLACK_MESSAGE_PRIORITIES.NORMAL,
  ...
  async addBulkSlackMessageJobs(
    ...
    const priority = options?.priority || SLACK_MESSAGE_PRIORITIES.NORMAL;
    const results: any[] = [];
  ...
  async retryFailedMessage(
    ...
    return this.slackMessageQueue.add('send-slack-message', enhancedData, {
      priority: SLACK_MESSAGE_PRIORITIES.HIGH, // Higher priority for retries
